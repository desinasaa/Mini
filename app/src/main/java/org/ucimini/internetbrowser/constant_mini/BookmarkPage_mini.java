 ...
 */
package org.ucimini.internetbrowser.constant_mini;
...
import org.ucimini.internetbrowser.favicon_mini.FaviconUtils_mini;
...
public final class BookmarkPage_mini {
    ...
    public BookmarkPage_mini(@NonNull Activity activity) {
    ...
    public Single<String> getBookmarkPage() {
        return Single.create(new SingleAction<String>() {
            ...
            public void onSubscribe(@NonNull SingleSubscriber<String> subscriber) {
                ...
                cacheIcon(mFaviconModelMini.getDefaultBitmapForString(null), getDefaultIconFile(mApp));
                buildBookmarkPage(null);

                File bookmarkWebPage = getBookmarkPage(mApp, null);

                subscriber.onItem(Constants_mini.FILE + bookmarkWebPage);
    ...
    private void buildBookmarkPage(@Nullable final String folder) {
        mBookmarkModel.getBookmarksFromFolderSorted(folder)
            .subscribe(new SingleOnSubscribe<List<HistoryItem_mini>>() {
                @Override
                public void onItem(@Nullable final List<HistoryItem_mini> list) {
    ...
    private void buildPageHtml(@NonNull List<HistoryItem_mini> bookmarksAndFolders, @Nullable String folder) {
        ...
        for (int n = 0, size = bookmarksAndFolders.size(); n < size; n++) {
            final HistoryItem_mini item = bookmarksAndFolders.get(n);
            bookmarkBuilder.append(PART1);
            if (item.isFolder()) {
                final File folderPage = getBookmarkPage(mApp, item.getTitle());
                bookmarkBuilder.append(Constants_mini.FILE).append(folderPage);
                bookmarkBuilder.append(PART2);
                bookmarkBuilder.append(folderIconPath);
                buildBookmarkPage(item.getTitle());
            } else {

                Uri bookmarkUri = FaviconUtils_mini.safeUri(item.getUrl());

                String faviconFileUrl;

                if (bookmarkUri != null) {
                    File faviconFile = FaviconModel_mini.getFaviconCacheFile(mApp, bookmarkUri);
                    if (!faviconFile.exists()) {
                        Bitmap defaultFavicon = mFaviconModelMini.getDefaultBitmapForString(item.getTitle());
                        mFaviconModelMini.cacheFaviconForUrl(defaultFavicon, item.getUrl()).subscribe();
                    }

                    faviconFileUrl = Constants_mini.FILE + faviconFile;
                } else {
                    faviconFileUrl = Constants_mini.FILE + getDefaultIconFile(mApp);
                }
