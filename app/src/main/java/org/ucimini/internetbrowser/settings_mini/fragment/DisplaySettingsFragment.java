...
public class DisplaySettingsFragment extends LightningPreferenceFragment implements Preference.OnPreferenceClickListener, Preference.OnPreferenceChangeListener {
    ...
    private void initPrefs() {
        ...
        mCurrentTheme = mPreferenceManagerMini.getUseTheme();
        ...
        cbStatus.setChecked(mPreferenceManagerMini.getHideStatusBarEnabled());
        cbFullScreen.setChecked(mPreferenceManagerMini.getFullScreenEnabled());
        cbViewPort.setChecked(mPreferenceManagerMini.getUseWideViewportEnabled());
        cbOverview.setChecked(mPreferenceManagerMini.getOverviewModeEnabled());
        cbReflow.setChecked(mPreferenceManagerMini.getTextReflowEnabled());
        cbBlackStatus.setChecked(mPreferenceManagerMini.getUseBlackStatusBar());
        cbDrawerTabs.setChecked(mPreferenceManagerMini.getShowTabsInDrawer(true));
        cbSwapTabs.setChecked(mPreferenceManagerMini.getBookmarksAndTabsSwapped());

        mTheme.setSummary(mThemeOptions[mPreferenceManagerMini.getUseTheme()]);
    }
    ...
    public boolean onPreferenceChange(@NonNull Preference preference, Object newValue) {
        ...
        switch (preference.getKey()) {
            case SETTINGS_HIDESTATUSBAR:
                mPreferenceManagerMini.setHideStatusBarEnabled(checked);
                return true;
            case SETTINGS_FULLSCREEN:
                mPreferenceManagerMini.setFullScreenEnabled(checked);
                return true;
            case SETTINGS_VIEWPORT:
                mPreferenceManagerMini.setUseWideViewportEnabled(checked);
                return true;
            case SETTINGS_OVERVIEWMODE:
                mPreferenceManagerMini.setOverviewModeEnabled(checked);
                return true;
            case SETTINGS_REFLOW:
                mPreferenceManagerMini.setTextReflowEnabled(checked);
                return true;
            case SETTINGS_DRAWERTABS:
                mPreferenceManagerMini.setShowTabsInDrawer(checked);
                return true;
            case SETTINGS_SWAPTABS:
                mPreferenceManagerMini.setBookmarkAndTabsSwapped(checked);
                return true;
            case SETTINGS_BLACK_STATUS:
                mPreferenceManagerMini.setUseBlackStatusBar(checked);
                return true;
            default:
                return false;
        }
    ...
    private void textSizePicker() {
        ...
        final int MAX = 5;
        bar.setMax(MAX);
        bar.setProgress(MAX - mPreferenceManagerMini.getTextSize());
        ...
        builder.setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface arg0, int arg1) {
                mPreferenceManagerMini.setTextSize(MAX - bar.getProgress());
            }
    ...
    private void themePicker() {
        ...
        picker.setPositiveButton(getResources().getString(R.string.action_ok),
            new DialogInterface.OnClickListener() {
                ...
                @Override
                public void onClick(DialogInterface dialog, int which) {
                    if (mCurrentTheme != mPreferenceManagerMini.getUseTheme()) {
