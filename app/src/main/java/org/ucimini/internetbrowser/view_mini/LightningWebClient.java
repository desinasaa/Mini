...
import org.ucimini.internetbrowser.adblock_mini.AdBlock_mini;
...
public class LightningWebClient extends WebViewClient {
    ...
    public WebResourceResponse shouldInterceptRequest(WebView view, @NonNull WebResourceRequest request) {
        if (mAdBlockMini.isAd(request.getUrl().toString())) {
    ...
    public void onPageFinished(@NonNull WebView view, String url) {
        if (view.isShown()) {
            mXUIControllerMini.updateUrl(url, false);
            mXUIControllerMini.setBackButtonEnabled(view.canGoBack());
            mXUIControllerMini.setForwardButtonEnabled(view.canGoForward());
            view.postInvalidate();
        }
        ...
        if (Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT &&
            ...
            view.evaluateJavascript(Constants_mini.JAVASCRIPT_INVERT_PAGE, null);
        }
    ...
    public void onPageStarted(WebView view, String url, Bitmap favicon) {
        ...
        if (mLightningView.isShown()) {
            mXUIControllerMini.updateUrl(url, true);
        ...
        if (view.isShown() && mLightningView.mPreferences.getTextReflowEnabled() &&
            ...
            if (changeInPercent > 2.5f && !mIsRunning) {
                mIsRunning = view.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        mZoomScale = newScale;
                        view.evaluateJavascript(Constants_mini.JAVASCRIPT_TEXT_REFLOW, new ValueCallback<String>() {
                            @Override
    ...
    private boolean isMailOrIntent(@NonNull String url, @NonNull WebView view) {
        if (url.startsWith("mailto:")) {
            ...
            File file = new File(url.replace(Constants_mini.FILE, ""));
