...
package org.ucimini.internetbrowser.view_mini;
...
public class LightningView {
    ...
    public LightningView(@NonNull Activity activity, @Nullable String url, boolean isIncognito) {
        ...
        mWebView.setDownloadListener(new LightningDownloadListener_mini(activity));
    ...
    /**
     * This method gets the startpage URL from the {@link StartPage_mini}
     * class asynchronously and loads the URL in the WebView on the
     * UI thread.
     */
    private void loadStartpage() {
        new StartPage_mini().getHomepage()
    ...
    /**
     * This method gets the bookmark page URL from the {@link BookmarkPage_mini}
     * class asynchronously and loads the URL in the WebView on the
     * UI thread. It also caches the default folder icon locally.
     */
    public void loadBookmarkpage() {
        new BookmarkPage_mini(mActivity).getBookmarkPage()
            .subscribeOn(Schedulers.io())
            .observeOn(Schedulers.main())
            .subscribe(new SingleOnSubscribe<String>() {
                @Override
    ...
    public void toggleDesktopUA(@NonNull Context context) {
        if (mWebView == null)
            return;
        if (!mToggleDesktop)
            mWebView.getSettings().setUserAgentString(Constants_mini.DESKTOP_USER_AGENT);
        else
            setUserAgent(context, mPreferences.getUserAgentChoice());
    ...
    private void setUserAgent(Context context, int choice) {
        ...
        switch (choice) {
            case 1:
                ...
                break;
            case 2:
                settings.setUserAgentString(Constants_mini.DESKTOP_USER_AGENT);
                break;
            case 3:
                settings.setUserAgentString(Constants_mini.MOBILE_USER_AGENT);
                break;
            case 4:
    ...
    public void setIsForegroundTab(boolean isForeground) {
        ...
        mXUIControllerMini.tabChanged(this);
    }
    ...
    private void longClickPage(@Nullable final String url) {
        ...
        if (currentUrl != null && UrlUtils.isSpecialUrl(currentUrl)) {
            if (UrlUtils.isHistoryUrl(currentUrl)) {
                if (url != null) {
                    mDialogBuilder.showLongPressedHistoryLinkDialog(mActivity, mXUIControllerMini, url);
                } else if (result != null && result.getExtra() != null) {
                    final String newUrl = result.getExtra();
                    mDialogBuilder.showLongPressedHistoryLinkDialog(mActivity, mXUIControllerMini, newUrl);
                }
            } else if (UrlUtils.isBookmarkUrl(currentUrl)) {
                if (url != null) {
                    mDialogBuilder.showLongPressedDialogForBookmarkUrl(mActivity, mXUIControllerMini, url);
                } else if (result != null && result.getExtra() != null) {
                    final String newUrl = result.getExtra();
                    mDialogBuilder.showLongPressedDialogForBookmarkUrl(mActivity, mXUIControllerMini, newUrl);
                }
            } else if (UrlUtils.isDownloadsUrl(currentUrl)) {
                if (url != null) {
                    mDialogBuilder.showLongPressedDialogForDownloadUrl(mActivity, mXUIControllerMini, url);
                } else if (result != null && result.getExtra() != null) {
                    final String newUrl = result.getExtra();
                    mDialogBuilder.showLongPressedDialogForDownloadUrl(mActivity, mXUIControllerMini, newUrl);
                }
            }
        } else {
            if (url != null) {
                if (result != null) {
                    if (result.getType() == WebView.HitTestResult.SRC_IMAGE_ANCHOR_TYPE || result.getType() == WebView.HitTestResult.IMAGE_TYPE) {
                        mDialogBuilder.showLongPressImageDialog(mActivity, mXUIControllerMini, url, getUserAgent());
                    } else {
                        mDialogBuilder.showLongPressLinkDialog(mActivity, mXUIControllerMini, url);
                    }
                } else {
                    mDialogBuilder.showLongPressLinkDialog(mActivity, mXUIControllerMini, url);
                }
                ...
                final String newUrl = result.getExtra();
                if (result.getType() == WebView.HitTestResult.SRC_IMAGE_ANCHOR_TYPE || result.getType() == WebView.HitTestResult.IMAGE_TYPE) {
                    mDialogBuilder.showLongPressImageDialog(mActivity, mXUIControllerMini, newUrl, getUserAgent());
                } else {
                    mDialogBuilder.showLongPressLinkDialog(mActivity, mXUIControllerMini, newUrl);
                }
            }
        }
    }
    ...
    public Bitmap getFavicon() {
        return mTitle.getFavicon(mXUIControllerMini.getUseDarkTheme());
    }
        ...
        public boolean onTouch(@Nullable View view, @NonNull MotionEvent arg1) {
            ...
            if (mAction == MotionEvent.ACTION_DOWN) {
                ...
                if (distance > SCROLL_UP_THRESHOLD && view.getScrollY() < SCROLL_UP_THRESHOLD) {
                    mXUIControllerMini.showActionBar();
                } else if (distance < -SCROLL_UP_THRESHOLD) {
                    mXUIControllerMini.hideActionBar();
                }
                mLocation = 0;
            }
