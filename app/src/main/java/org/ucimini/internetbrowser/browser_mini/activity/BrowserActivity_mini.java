...
package org.ucimini.internetbrowser.browser_mini.activity;
...
import org.ucimini.internetbrowser.browser_mini.BrowserView_mini;
import org.ucimini.internetbrowser.browser_mini.TabsManager_mini;
import org.ucimini.internetbrowser.browser_mini.TabsView_mini;
...
import org.ucimini.internetbrowser.incognitoActivity;
...
import org.ucimini.internetbrowser.controller_mini.UIController_mini;
...
import org.ucimini.internetbrowser.interpolator_mini.BezierDecelerateInterpolator_mini;
import org.ucimini.internetbrowser.receiver_mini.NetworkReceiver_mini;
...
import org.ucimini.internetbrowser.search.engine.BaseSearchEngine;
import org.ucimini.internetbrowser.settings_mini.activity.SettingsActivityMini;
import org.ucimini.internetbrowser.utils.DrawableUtils;
import org.ucimini.internetbrowser.utils.IntentUtils;
import org.ucimini.internetbrowser.utils.Preconditions;
...
import org.ucimini.internetbrowser.utils.WebUtils;
import org.ucimini.internetbrowser.view_mini.Handlers;
import org.ucimini.internetbrowser.view_mini.LightningView;
import org.ucimini.internetbrowser.view_mini.SearchView;
...
public abstract class BrowserActivity_mini extends ThemableBrowserActivity implements BrowserView_mini, UIController_mini, OnClickListener {

    private static final String TAG = "BrowserActivity_mini";
    ...
    @Inject
    SearchBoxModel_mini mSearchBoxModelMini;

    @Inject SearchEngineProvider mSearchEngineProvider;

    private TabsManager_mini mTabsManagerMini;
    ...
    private BrowserPresenter_mini mPresenter;
    private TabsView_mini mTabsViewMini;
    private BookmarksView_mini mBookmarksViewMini;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //StartAppSDK.init(this, "204417647", true);
        //StartAppAd.disableSplash();
        ButterKnife.bind(this);

        mTabsManagerMini = new TabsManager_mini();
        mPresenter = new BrowserPresenter_mini(this, isIncognito());
    ...
    private synchronized void initialize(Bundle savedInstanceState) {
        ...
        TabsFragment_miniMini tabsFragment = (TabsFragment_miniMini) fragmentManager.findFragmentByTag(TAG_TABS_FRAGMENT);
        BookmarksFragment_miniMini bookmarksFragment = (BookmarksFragment_miniMini) fragmentManager.findFragmentByTag(TAG_BOOKMARK_FRAGMENT);

        if (tabsFragment != null) {
            fragmentManager.beginTransaction().remove(tabsFragment).commit();
        }
        tabsFragment = TabsFragment_miniMini.createTabsFragment(isIncognito(), mShowTabsInDrawer);

        mTabsViewMini = tabsFragment;

        if (bookmarksFragment != null) {
            fragmentManager.beginTransaction().remove(bookmarksFragment).commit();
        }
        bookmarksFragment = BookmarksFragment_miniMini.createFragment(isIncognito());

        mBookmarksViewMini = bookmarksFragment;
    ...
    protected void panicClean() {
        ...
        mTabsManagerMini.newTab(this, "", false);
        mTabsManagerMini.switchToTab(0);
        mTabsManagerMini.clearSavedState();
        HistoryPage_mini.deleteHistoryPage(getApplication()).subscribe();
        closeBrowser();
        ...
        public boolean onKey(View searchView, int keyCode, KeyEvent keyEvent) {
            ...
            switch (keyCode) {
                case KeyEvent.KEYCODE_ENTER:
                    ...
                    final LightningView currentView = mTabsManagerMini.getCurrentTab();
                    if (currentView != null) {
    ...
    private void initializePreferences() {
        ...
        if (tabsFragment instanceof TabsFragment_miniMini) {
            ((TabsFragment_miniMini) tabsFragment).reinitializePreferences();
        }
        Fragment bookmarksFragment = manager.findFragmentByTag(TAG_BOOKMARK_FRAGMENT);
        if (bookmarksFragment instanceof BookmarksFragment_miniMini) {
            ((BookmarksFragment_miniMini) bookmarksFragment).reinitializePreferences();
        }
...
//        mBrowserFrame.setLayoutTransition(new LayoutTransition());
    ...
    private final Runnable mLongPressBackRunnable = new Runnable() {
        @Override
        public void run() {
            final LightningView currentTab = mTabsManagerMini.getCurrentTab();
            showCloseDialog(mTabsManagerMini.positionOf(currentTab));
        }
    };
            ...
            if (event.isCtrlPressed()) {
                switch (event.getKeyCode()) {
                    ...
                    case KeyEvent.KEYCODE_W:
                        // Close current tab
                        mPresenter.deleteTab(mTabsManagerMini.indexOfCurrentTab());
                        return true;
                    ...
                    case KeyEvent.KEYCODE_R:
                        // Refresh current tab
                        LightningView currentTab = mTabsManagerMini.getCurrentTab();
                        if (currentTab != null) {
                            currentTab.reload();
                        }
                        return true;
                    case KeyEvent.KEYCODE_TAB:
                        ...
                        if (event.isShiftPressed()) {
                            // Go back one tab
                            if (mTabsManagerMini.indexOfCurrentTab() > 0) {
                                nextIndex = mTabsManagerMini.indexOfCurrentTab() - 1;
                            } else {
                                nextIndex = mTabsManagerMini.last();
                            }
                        } else {
                            // Go forward one tab
                            if (mTabsManagerMini.indexOfCurrentTab() < mTabsManagerMini.last()) {
                                nextIndex = mTabsManagerMini.indexOfCurrentTab() + 1;
                            } else {
                                nextIndex = 0;
                            }
                ...
                if (KeyEvent.KEYCODE_0 <= event.getKeyCode() && event.getKeyCode() <= KeyEvent.KEYCODE_9) {
                    int nextIndex;
                    if (event.getKeyCode() > mTabsManagerMini.last() + KeyEvent.KEYCODE_1 || event.getKeyCode() == KeyEvent.KEYCODE_0) {
                        nextIndex = mTabsManagerMini.last();
                    } else {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_add_to_homescreen:
                if (currentView != null) {
                    HistoryItem_mini shortcut = new HistoryItem_mini(currentView.getUrl(), currentView.getTitle());
            ...
            case R.id.action_settings:
                startActivity(new Intent(this, SettingsActivityMini.class));
                return true;
            ...
            case R.id.action_reading_mode:
                if (currentUrl != null) {
                    ...
                    read.putExtra(Constants_mini.LOAD_READING_URL, currentUrl);
                    startActivity(read);
                }
                return true;
            default:
    ...
    private void addBookmark(final String title, final String url) {
        ...
        final HistoryItem_mini item = new HistoryItem_mini(url, title);
            ...
            .subscribe(new SingleOnSubscribe<Boolean>() {
                ...
                public void onItem(@Nullable Boolean item) {
                    if (Boolean.TRUE.equals(item)) {
                        ...
                        mBookmarksViewMini.handleUpdatedUrl(url);
                    }
                }
            });
    }
    ...
    @Override
    public TabsManager_mini getTabModel() {
        return mTabsManagerMini;
    }
    ...
    @Override
    public void bookmarkItemClicked(@NonNull HistoryItem_mini item) {
    ...
    public void closeBrowser() {
        ...
        removeViewFromParent(mCurrentView);
        performExitCleanUp();
        int size = mTabsManagerMini.size();
        mTabsManagerMini.shutdown();
        mCurrentView = null;
        for (int n = 0; n < size; n++) {
            mTabsViewMini.tabRemoved(0);
        }
        finish();
    }
    ...
    public synchronized void onBackPressed() {
        ...
        if (mDrawerLayout.isDrawerOpen(getTabDrawer())) {
            ...
            mBookmarksViewMini.navigateBack();
        } else {
            if (currentTab != null) {
                ...
                if (mSearch.hasFocus()) {
                    ...
                    if (mCustomView != null || mCustomViewCallback != null) {
                        ...
                        mPresenter.deleteTab(mTabsManagerMini.positionOf(currentTab));
                    }
                }
            } else {
    ...
    protected void onPause() {
        super.onPause();
        Log.d(TAG, "onPause");
        mTabsManagerMini.pauseAll();
        try {
            getApplication().unregisterReceiver(mNetworkReceiverMini);
    ...
    protected void onResume() {
        ...
        mTabsManagerMini.resumeAll(this);
        ...
        getApplication().registerReceiver(mNetworkReceiverMini, filter);
    ...
    public void updateUrl(@Nullable String url, boolean isLoading) {
        ...
        mSearch.setText(mSearchBoxModelMini.getDisplayContent(url, currentTitle, isLoading));
    }
    ...
    private void openHistory() {
        new HistoryPage_mini().getHistoryPage()
            .subscribeOn(Schedulers.io())
            .observeOn(Schedulers.main())
            .subscribe(new SingleOnSubscribe<String>() {
                ...
                public void onItem(@Nullable String item) {
                    ...
                    LightningView view = mTabsManagerMini.getCurrentTab();
                    if (view != null) {
                        view.loadUrl(item);
                    }
                }
    ...
    private void openDownloads() {
        new DownloadsPage_mini().getDownloadsPage()
            .subscribeOn(Schedulers.io())
            .observeOn(Schedulers.main())
            .subscribe(new SingleOnSubscribe<String>() {
                @Override
    ...
    public synchronized void onCreateWindow(Message resultMsg) {
        ...
        if (newTab("", true)) {
            LightningView newTab = mTabsManagerMini.getTabAtPosition(mTabsManagerMini.size() - 1);
            if (newTab != null) {
    ...
    public void onCloseWindow(LightningView view) {
        mPresenter.deleteTab(mTabsManagerMini.positionOf(view));
    }
    ...
    public void hideActionBar() {
        if (mFullScreen) {
            ...
            if (mToolbarLayout.getTranslationY() > -0.01f) {
                ...
                show.setInterpolator(new BezierDecelerateInterpolator_mini());
    ...
    public void handleBookmarksChange() {
        ...
        if (currentTab != null) {
            mBookmarksViewMini.handleUpdatedUrl(currentTab.getUrl());
        }
    }
    ...
    @Override
    public void handleBookmarkDeleted(@NonNull HistoryItem_mini item) {
        mBookmarksViewMini.handleBookmarkDeleted(item);
    ...
    public void handleNewTab(@NonNull LightningDialogBuilder.NewTab newTabType, @NonNull String url) {
        ...
        switch (newTabType) {
            ...
            case BACKGROUND:
                ...
                break;
            case INCOGNITO:
                Intent intent = new Intent(BrowserActivity_mini.this, incognitoActivity.class);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()) {
            ...
            case R.id.action_reading:
                Intent read = new Intent(this, ReadingActivity.class);
                read.putExtra(Constants_mini.LOAD_READING_URL, currentTab.getUrl());
                startActivity(read);
                break;
    ...
    /**
     * This NetworkReceiver_mini notifies each of the WebViews in the browser whether
     ...
     */
    private final NetworkReceiver_mini mNetworkReceiverMini = new NetworkReceiver_mini() {
        @Override
        public void onConnectivityChange(boolean isConnected) {
            Log.d(TAG, "Network Connected: " + isConnected);
            mTabsManagerMini.notifyConnectionStatus(isConnected);
        }
    };
