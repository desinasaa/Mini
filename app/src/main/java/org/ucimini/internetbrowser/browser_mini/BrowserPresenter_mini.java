 ...
 * of the
 */
public class BrowserPresenter_mini {

    private static final String TAG = "BrowserPresenter_mini";

    @NonNull private final TabsManager_mini mTabsModel;
    @Inject Application mApplication;
    @Inject
    PreferenceManager_mini mPreferences;

    @NonNull private final BrowserView_mini mView;
    @Nullable private LightningView mCurrentTab;

    private final boolean mIsIncognito;
    private boolean mShouldClose;

    public BrowserPresenter_mini(@NonNull BrowserView_mini view, boolean isIncognito) {
        BrowserApp.getAppComponent().inject(this);
        mTabsModel = ((UIController_mini) view).getTabModel();
        mView = view;
        mIsIncognito = isIncognito;
        mTabsModel.setTabNumberChangedListener(new TabsManager_mini.TabNumberChangedListener() {
            @Override
     ...
     * that is handed in by the BrowserActivity_mini.
     *
    ...
    private String mapHomepageToCurrentUrl() {
        ...
        switch (homepage) {
            case Constants_mini.SCHEME_HOMEPAGE:
                return Constants_mini.FILE + StartPage_mini.getStartPageFile(mApplication);
            case Constants_mini.SCHEME_BOOKMARKS:
                return Constants_mini.FILE + BookmarkPage_mini.getBookmarkPage(mApplication, null);
            default:
                return homepage;
        }
    }
...
//            }
        ...
        mTabsModel.doAfterInitialization(new Runnable() {
            ...
            public void run() {
                ...
                if (intent != null && intent.getExtras() != null) {
                    tabHashCode = intent.getExtras().getInt(Constants_mini.INTENT_ORIGIN);
                }
     ...
     * the BrowserActivity_mini is destroyed so that
