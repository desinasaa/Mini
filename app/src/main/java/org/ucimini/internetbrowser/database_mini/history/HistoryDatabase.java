 ...
 */
package org.ucimini.internetbrowser.database_mini.history;
...
public class HistoryDatabase extends SQLiteOpenHelper implements HistoryModel {
    ...
    @NonNull
    private static HistoryItem_mini fromCursor(@NonNull Cursor cursor) {
        ...
        return Completable.create(new CompletableAction() {
            ...
            public void onSubscribe(@NonNull CompletableSubscriber subscriber) {
                ...
                if (cursor.getCount() > 0) {
                    ...
                    addHistoryItem(new HistoryItem_mini(url, title == null ? "" : title));
                }
    ...
    @NonNull
    @Override
    public Single<List<HistoryItem_mini>> findHistoryItemsContaining(@NonNull final String query) {
        return Single.create(new SingleAction<List<HistoryItem_mini>>() {
            ...
            public void onSubscribe(@NonNull SingleSubscriber<List<HistoryItem_mini>> subscriber) {
                List<HistoryItem_mini> itemList = new ArrayList<>(5);
    ...
    public Single<List<HistoryItem_mini>> lastHundredVisitedHistoryItems() {
        return Single.create(new SingleAction<List<HistoryItem_mini>>() {
            @Override
            public void onSubscribe(@NonNull SingleSubscriber<List<HistoryItem_mini>> subscriber) {
                List<HistoryItem_mini> itemList = new ArrayList<>(100);
    ...
    @WorkerThread
    private synchronized void addHistoryItem(@NonNull HistoryItem_mini item) {
        ContentValues values = new ContentValues();
    ...
    synchronized List<HistoryItem_mini> getAllHistoryItems() {
        List<HistoryItem_mini> itemList = new ArrayList<>();
