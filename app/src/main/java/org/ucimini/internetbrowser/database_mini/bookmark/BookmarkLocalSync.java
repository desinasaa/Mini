...
public class BookmarkLocalSync {
    ...
    @NonNull
    private List<HistoryItem_mini> getBookmarksFromContentUri(String contentUri) {
        List<HistoryItem_mini> list = new ArrayList<>();
            ...
            if (cursor != null) {
                ...
                while (cursor.moveToNext()) {
                    if (cursor.getInt(2) == 1) {
                        ...
                        if (title != null) {
                            list.add(new HistoryItem_mini(url, title));
                        }
                    }
                }
            }
    ...
    @NonNull
    @WorkerThread
    public List<HistoryItem_mini> getBookmarksFromStockBrowser() {
        return getBookmarksFromContentUri(STOCK_BOOKMARKS_CONTENT);
    }

    @NonNull
    @WorkerThread
    public List<HistoryItem_mini> getBookmarksFromChrome() {
        return getBookmarksFromContentUri(CHROME_BOOKMARKS_CONTENT);
    }

    @NonNull
    @WorkerThread
    public List<HistoryItem_mini> getBookmarksFromChromeBeta() {
        return getBookmarksFromContentUri(CHROME_BETA_BOOKMARKS_CONTENT);
    }

    @NonNull
    @WorkerThread
    public List<HistoryItem_mini> getBookmarksFromChromeDev() {
